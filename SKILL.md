...
Use source images before PDF crops when arXiv source is available. `scripts/extract_tex_source.py` reports `image_files`, `graphics_refs`, and `source_image_assets`; pass `--convert-images` when preparing Markdown-visible assets from source PDF/EPS figures. Prefer these original source assets for framework diagrams, architecture diagrams, and result plots when they match the final PDF.
